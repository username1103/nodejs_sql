...
exports.page = function(req, res){
	...
	let sort = queryData.sort === undefined  ? 'created' : queryData.sort;

	db.query(`SELECT * FROM topic ORDER BY ${sort}`, (err, topics)=>{
		if(err) throw err;
	
		let list = template.getList(topics,sort);
		let control = template.getControl(queryData.id);
		db.query('SELECT * FROM topic LEFT JOIN author ON topic.author_id = author.id WHERE topic.id=?',[queryData.id], (err2, result) => {
			...
			let html = template.getHTML(title, list, body, control);
